...
void AirHockey::begin_pos()
{
	...
	_pieces[0] = { _pieces[0].score, wid / 2, hei , wid / 2, hei , 0, 0 };
	_pieces[1] = { _pieces[1].score, wid / 2 + wid , hei, wid / 2 + wid, hei, 0, 0 };
...
void AirHockey::behav_puck()
{
	...
	// goal 
	if ((puck.x < 85 || puck.x > WIDTH - 85)  && (puck.y > 260 && puck.y < 477))
	{
		_pieces[(puck.x < 85  ? tbot : tplayer)].score++;
...
void AirHockey::behav_bot()
{
	piece & bot = _pieces[tbot];
	piece & puck = _pieces[tpuck];
	double speed = (_humanMode ? MAX_SPEED / 3: MAX_SPEED / 6);
	bot.xp = bot.x;
	bot.yp = bot.y;
...
{
	_lib->new_game(_humanMode);
	while (true)
	{
		_event = _lib->checkEvent(_pieces[1], _pieces[0]);
		switch (_event)
		{
		case nothing:
			break;
